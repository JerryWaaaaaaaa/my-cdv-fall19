...
d3.json("monarchs.json").then(function(incomingData){
  ...
  }).selectAll("circle").attr("class", "diplomacycircle");
  ...
  datagroups
    .on("mouseover", function(d){
        ...
        element.select("text").transition().duration(500).attr("x", 30).attr("opacity", 1);
    ...
    .on("mouseout", function(){
        ...
        element.select("text").transition().duration(500).attr("x", 0).attr("opacity", 0);
        element.select("text").text(function(d){
          return " ";
        })
    })
